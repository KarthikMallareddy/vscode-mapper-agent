...
export function activate(context: vscode.ExtensionContext) {
    ...
    const mapper = vscode.chat.createChatParticipant("mapper", async (request, context_chat, response, token) => {
        ...
        const model = new ChatGoogleGenerativeAI({
            // 1. Move to the 2.5 series to resolve the 404 (Availability)
            model: "gemini-2.5-flash", 
            apiKey: apiKey,
            // 2. Keep v1beta to ensure the newest 2.5 features are supported
            apiVersion: "v1beta", 
        });
        ...
        if (request.command === 'draw') {
            ...
            // Inside your request.command === 'draw' block
try {
    const aiResponse = await model.invoke([...]);
    const mermaidCode = aiResponse.content as string;

    // 1. Show it in the chat as backup
    response.markdown("### 🗺️ System Architecture Map\n\n" + mermaidCode);

    // 2. OPEN THE CLEAR DIAGRAM ON THE SCREEN
...
function openMermaidPreview(mermaidCode: string) {
    const panel = vscode.window.createWebviewPanel(
        'mermaidPreview',
        'Architecture Map',
        vscode.ViewColumn.Two, // Open it to the side
        { enableScripts: true }
    );
