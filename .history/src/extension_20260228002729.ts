...
export function activate(context: vscode.ExtensionContext) {
    ...
    const mapper = vscode.chat.createChatParticipant("mapper", async (request, context_chat, response, token) => {
        ...
        if (request.command === 'draw') {
            response.markdown("🎨 **@mapper is analyzing your project architecture...**\n\n");
            const workspaceFolders = vscode.workspace.workspaceFolders;
            if (!workspaceFolders) return;

            const fileTree = await getFileTree(workspaceFolders[0].uri.fsPath);
            const aiResponse = await model.invoke([
                ["system", "Analyze the file tree and generate a high-level architecture diagram using Mermaid.js syntax (graph TD). Group logic into layers."],
